...
// post
  function createFruit(data,id, callback){
    ...
    .then(function(response) {
      if(!response.ok) alert('Thêm mới không thành công.')
      else{
...
function onclickButtonSaveEdit(){
  ...
  window.location="http://127.0.0.1:5501/pages/Hien_thi/Fruit.html"; // khi chuyển folder phải sửa lại
  sessionStorage.removeItem('idFruitEdit');
  sessionStorage.removeItem('idFruitEdit');
