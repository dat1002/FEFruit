...
function userLogin(data) {
  ...
  fetch(login, options)
    .then(function (response) {
      if (!response.ok) alert("Tài khoản hoặc mật khẩu của bạn chưa chính xác. Vui lòng nhập lại!");
      else {
    ...
    .then(function (data) {
      ...
      localStorage.setItem("userRole", data.result.roles);

      var checkRole = localStorage.getItem("userRole");
      console.log(checkRole=="ROLE_USER2");
      console.log(checkRole);
      if(checkRole == "ROLE_USER2"){
        window.location = "http://127.0.0.1:5501/pages/nhanvienkho/ChangeInfNV.html";
      }
      else
        window.location = "http://127.0.0.1:5501/pages/Hien_thi/home.html";
      handleUsersById(parseInt(localStorage.getItem("userId")));
    });
}
...
function onClickButtonSignOut() {
  localStorage.clear();
  window.location = "../Hien_thi/login.html";
}
