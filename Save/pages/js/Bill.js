        ...
        <button type="button" class="btn btn-info show-bd">
          ...
          <div class="bill-details" id="bill-details-${bill.id}" >
            <div class="form-bill-details">
              <div class="title m-3"> <h2>CHI TIẾT HÓA ĐƠN</h2></div>
              <h6>Ngày: ${bill.dateCreated}</h6>
                <div class="inf-customer">
                  <div> 
                    <b>Họ tên</b>  <span>: ${bill.customer.name}</span> <br>
                    <b>Email</b>      <span>:  ${bill.customer.email}</span> <br>
                    <b>Code</b>      <span>:  ${bill.code}</span> <br>
                  </div> 

                  <div>
                    <b>Địa chỉ</b> <span>:  ${bill.customer.address}</span>
                    <br>

                    <b>Số điện thoại</b> <span>:  ${bill.customer.phoneNumber}</span>
                  </div>
                </div>
                <div class="buy-list">
                ...
                <div>
                  <h6 style="margin: 25px;">
                    Người tạo: <span>${bill.user.name}</span>
                  </h6>
                </div>
              </div> 
          </div>
        ...
        <div class="bill-details" id="bill-details-${bill.result.id}" >
          <div class="form-bill-details">
            ...
            <h6>Date: ${bill.result.dateCreated}</h6>
              <div class="inf-customer">
                <div> 
                  <b>Họ tên</b>  <span>: ${bill.result.customer.name}</span> <br>
                  <b>Email</b>      <span>:  ${bill.result.customer.email}</span> <br>
                  <b>Code</b>      <span>:  ${bill.result.code}</span> <br>
                </div> 

                <div>
                  <b>Địa chỉ</b> <span>:  ${bill.result.customer.address}</span>
                  <br>

                  <b>Số điện thoại</b> <span>:  ${bill.result.customer.phoneNumber}</span>
                </div>
              </div>
              <div class="buy-list">
                <form class="" id="" name="" action="" method="">
              ...
              <div>
                <h6 style="margin: 25px;">
                  Người tạo: <span>${bill.result.user.name}</span>
                </h6>
              </div>
            </div> 
        </div>
...
function createBill(data,idUser,idCustomer, callback){
  ...
  .then(function(response) {
    if(!response.ok) alert('Thêm mới thất bại!!')
    else{
